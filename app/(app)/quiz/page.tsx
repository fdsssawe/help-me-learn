...
export default async function QuizPage({
  ...
  const [yesterdayCount, weekCount, totalCount, languages] = await Promise.all([
    ...
    prisma.word.count({ where: { ...base, createdAt: { gte: subDays(now, 7) } } }),
    prisma.word.count({ where: base }),
    getLanguages(),
  ])
    ...
    <div className="animate-fade-in max-w-3xl mx-auto px-4 py-8">
      ...
      {totalCount === 0 ? (
        <div className="py-14 text-center">
          <div className="flex justify-center mb-4 text-text-muted"><BookOpen size={52} strokeWidth={1.25} /></div>
          {style === "sentences" ? (
            <>
              <h2 className="font-display text-[clamp(1.5rem,4vw,1.9rem)] text-text mb-3">No sentence-ready words yet</h2>
              <p className="text-text-muted mb-6 leading-relaxed max-w-md mx-auto">
                Sentence quizzes use example sentences, which come automatically when you add Italian or Spanish words. Add some, or switch to <strong>Words</strong> above.
              </p>
            </>
          ) : (
            <>
              <h2 className="font-display text-[clamp(1.5rem,4vw,1.9rem)] text-text mb-3">
        ...
        <>
          ...
          <p className="mt-8 text-[0.82rem] text-text-muted text-center">
            ...
            {style === "sentences" ? " with example sentences" : ""}
